...
int main(int argc, char *argv[])
{
    ...
    if (argc != 16)
    {
        std::cout << "Usage: mpirun -n <size> ./fem_simulation <scale> <use_ofem> <domain_size_x> <domain_size_y> <domain_size_z> <duration> <source_position_x> <source_position_y> <source_position_z> <observation_position_x> <observation_position_y> <observation_position_z> <dim_x> <dim_y> <dim_z>" << std::endl;
    ...
    source_position_x = std::stod(argv[7]);
    source_position_y = std::stod(argv[8]);
    source_position_z = std::stod(argv[9]);
    observation_position_x = std::stod(argv[10]);
    observation_position_y = std::stod(argv[11]);
    observation_position_z = std::stod(argv[12]);
    dim_x = std::stoi(argv[13]);
    dim_y = std::stoi(argv[14]);
    dim_z = std::stoi(argv[15]);

    domain_sizes[0] = domain_size_x;
    domain_sizes[1] = domain_size_y;
    domain_sizes[2] = domain_size_z;
    grid_nums[0] = scale * 100;
    grid_nums[1] = scale * 100;
    grid_nums[2] = scale * 100;
    ...
    time_step = domain_size / (c * std::sqrt(3.0));
    ...
    MPI_Comm_split(MPI_COMM_WORLD, (coords[0] + 1) / 2, (coords[0] + 1) % 2, &comm_x_plane_0);
    MPI_Comm_rank(comm_x_plane_0, &rank_x_plane_0);
    MPI_Comm_split(MPI_COMM_WORLD, coords[0] / 2, coords[0] % 2, &comm_x_plane_1);
    MPI_Comm_rank(comm_x_plane_1, &rank_x_plane_1);
    MPI_Comm_split(MPI_COMM_WORLD, (coords[1] + 1) / 2, (coords[1] + 1) % 2, &comm_y_plane_0);
    MPI_Comm_rank(comm_y_plane_0, &rank_y_plane_0);
    MPI_Comm_split(MPI_COMM_WORLD, coords[1] / 2, coords[1] % 2, &comm_y_plane_1);
    MPI_Comm_rank(comm_y_plane_1, &rank_y_plane_1);
    MPI_Comm_split(MPI_COMM_WORLD, (coords[2] + 1) / 2, (coords[2] + 1) % 2, &comm_z_plane_0);
    MPI_Comm_rank(comm_z_plane_0, &rank_z_plane_0);
    MPI_Comm_split(MPI_COMM_WORLD, coords[2] / 2, coords[2] % 2, &comm_z_plane_1);
    MPI_Comm_rank(comm_z_plane_1, &rank_z_plane_1);
    MPI_Comm_split(MPI_COMM_WORLD, (coords[1] + 1) / 2 + ((coords[2] + 1) / 2) * dims[1], (coords[1] + 1) % 2 + ((coords[2] + 1) % 2) * 2, &comm_x_line_0);
    MPI_Comm_rank(comm_x_line_0, &rank_x_line_0);
    MPI_Comm_split(MPI_COMM_WORLD, coords[1] / 2 + ((coords[2] + 1) / 2) * dims[1], coords[1] % 2 + ((coords[2] + 1) % 2) * 2, &comm_x_line_1);
    MPI_Comm_rank(comm_x_line_1, &rank_x_line_1);
    MPI_Comm_split(MPI_COMM_WORLD, (coords[1] + 1) / 2 + (coords[2] / 2) * dims[1], (coords[1] + 1) % 2 + (coords[2] % 2) * 2, &comm_x_line_2);
    MPI_Comm_rank(comm_x_line_2, &rank_x_line_2);
    MPI_Comm_split(MPI_COMM_WORLD, coords[1] / 2 + (coords[2] / 2) * dims[1], coords[1] % 2 + (coords[2] % 2) * 2, &comm_x_line_3);
    MPI_Comm_rank(comm_x_line_3, &rank_x_line_3);
    MPI_Comm_split(MPI_COMM_WORLD, (coords[2] + 1) / 2 + ((coords[0] + 1) / 2) * dims[0], (coords[2] + 1) % 2 + ((coords[0] + 1) % 2) * 2, &comm_y_line_0);
    MPI_Comm_rank(comm_y_line_0, &rank_y_line_0);
    MPI_Comm_split(MPI_COMM_WORLD, coords[2] / 2 + ((coords[0] + 1) / 2) * dims[0], coords[2] % 2 + ((coords[0] + 1) % 2) * 2, &comm_y_line_1);
    MPI_Comm_rank(comm_y_line_1, &rank_y_line_1);
    MPI_Comm_split(MPI_COMM_WORLD, (coords[2] + 1) / 2 + (coords[0] / 2) * dims[0], (coords[2] + 1) % 2 + (coords[0] % 2) * 2, &comm_y_line_2);
    MPI_Comm_rank(comm_y_line_2, &rank_y_line_2);
    MPI_Comm_split(MPI_COMM_WORLD, coords[2] / 2 + (coords[0] / 2) * dims[0], coords[2] % 2 + (coords[0] % 2) * 2, &comm_y_line_3);
    MPI_Comm_rank(comm_y_line_3, &rank_y_line_3);
    MPI_Comm_split(MPI_COMM_WORLD, (coords[0] + 1) / 2 + ((coords[1] + 1) / 2) * dims[0], (coords[0] + 1) % 2 + ((coords[1] + 1) % 2) * 2, &comm_z_line_0);
    MPI_Comm_rank(comm_z_line_0, &rank_z_line_0);
    MPI_Comm_split(MPI_COMM_WORLD, coords[0] / 2 + ((coords[1] + 1) / 2) * dims[0], coords[0] % 2 + ((coords[1] + 1) % 2) * 2, &comm_z_line_1);
    MPI_Comm_rank(comm_z_line_1, &rank_z_line_1);
    MPI_Comm_split(MPI_COMM_WORLD, (coords[0] + 1) / 2 + (coords[1] / 2) * dims[0], (coords[0] + 1) % 2 + (coords[1] % 2) * 2, &comm_z_line_2);
    MPI_Comm_rank(comm_z_line_2, &rank_z_line_2);
    MPI_Comm_split(MPI_COMM_WORLD, coords[0] / 2 + (coords[1] / 2) * dims[0], coords[0] % 2 + (coords[1] % 2) * 2, &comm_z_line_3);
    ...
    std::cout << "Using GPU device: " << gpu_id << " out of " << num_gpus << " available GPUs." << std::endl;

    // シミュレーションオブジェクトの作成
    start_time = omp_get_wtime();
    FemSimulation simulation(grid_nums, domain_size, time_step, permittivity, permeability,
                             time_frequency, use_ofem, dims, coords,
