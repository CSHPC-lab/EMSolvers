...
void FemSimulation::calculateElementStiffnessMatrix()
{
    ...
    Kmat_Eigen = 4.0 * time_step_ * time_step_ / (domain_size_ * order_) / (domain_size_ * order_) / permeability_ / permittivity_ / (8.0 / order_ / order_ / order_) * Kmat_Eigen;
...
#pragma acc parallel loop
    ...
    dispatchOuterElements(
        ...
        ef_x_idx_1_, ef_x_idx_2_,
        ef_y_idx_1_, ef_y_idx_2_,
        ef_z_idx_1_, ef_z_idx_2_,
        OE, mat_size_);
...
#pragma acc parallel loop
    ...
    {
        double temp = source_function(current_time_, permeability_, domain_size_) * time_step_ *
