...
class FemSimulation {
   public:
    // コンストラクタ
    FemSimulation(double domain_size, const std::array<int, 3>& grid_size, double time_step,
    ...
    // 結果の保存
    void saveResults(double observe_r, std::function<double(double)> source_function, int num_steps,
                     double c);
   ...
   private:
    ...
    int time_frequency_;
    // 電場の配列
    std::vector<std::vector<double>> electric_field_x_;
    std::vector<std::vector<double>> electric_field_y_;
    std::vector<std::vector<double>> electric_field_z_;

    // コネクティビティ
    std::vector<std::vector<int>> connectivity_x_;
    std::vector<std::vector<int>> connectivity_y_;
    std::vector<std::vector<int>> connectivity_z_;

    // 外力の情報
    std::vector<int> source_position_x_;
    std::vector<std::function<double(double)>> source_function_x_;
    std::vector<int> source_position_y_;
    std::vector<std::function<double(double)>> source_function_y_;
    std::vector<int> source_position_z_;
    std::vector<std::function<double(double)>> source_function_z_;
...
bool check_params(std::array<double, 3> domain_sizes, std::array<int, 3> grid_size, double duration,
